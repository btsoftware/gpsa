...
<?php		exit;
		...
		if ( ( empty( $redirect_to ) || $redirect_to == 'wp-admin/' || $redirect_to == admin_url() ) ) {
			...
			if ( is_multisite() && !get_active_blog_for_user($user->ID) && !is_super_admin( $user->ID ) ) die("1");
				$redirect_to = user_admin_url();
			elseif ( is_multisite() && !$user->has_cap('read') ) die("2");
				$redirect_to = get_dashboard_url( $user->ID );
			elseif ( !$user->has_cap('edit_posts') ) die("3");
